...
class SolidObjectsPayloadElement extends HTMLElement {
  ...
  deliver() {
    try {
      ...
      scope.dispatchEvent(new CustomEvent("solid-objects:payload", {
        bubbles: true,
        detail: { name, instanceId: revision[0], revision: revision[1], payload }
      }))
    } catch {
      this.dispatchEvent(new CustomEvent("solid-objects:payload-error", {
        bubbles: true,
...
function revisionFor(element) {
  const values = element.dataset.revision?.split(":").map(Number)
  if (!values || values.length !== 2 || values.some((value) => !Number.isSafeInteger(value) || value < 0)) return
  return values
}
